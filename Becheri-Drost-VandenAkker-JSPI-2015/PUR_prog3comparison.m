function PUR_prog3comparison(beta,hmax,hmin)
%calculates preliminary power curve PO and some top functions for optimal h0 in range [hmin,hmax] (warning provided in case boundary is hitted)
...
if nargin<2
    hmax=ceil(sqrt(9/(1+beta))/stepsize)*stepsize; %hmaxPE
end
if nargin<3
    hmin=floor(sqrt(7.5/(1+beta))/stepsize)*stepsize; %0
end
...
h00opt=-99;
diffh00opt=99;
for uu=1:hgridpointR-hgridpointL+1
...
load PObeta.mat PO TOPL TOPR h0
